...
tweets %>% 
  arrange(created_at) %>% 
  #将 tweets 数据框按照 created_at列进行升序排序，即最早的推文排在前面，最晚的推文排在后面
  tail(5) %>%
  #tail() 是 R 语言中的一个基本函数，用于获取数据框的最后几行
  kbl() %>% #用于将数据框转换为 Markdown 格式的表格
  kable_styling(c("striped", "hover", "condensed", "responsive"))
                                                                                                     #kable_styling()，用于美化 Markdown表格。"striped"：为表格添加条纹样式，使表格的奇偶行有不同的背景颜色，提高可读性。
#"hover"：当鼠标悬停在表格行上时，该行会有高亮显示。
#"condensed"：使表格的列间距变小，表格更加紧凑。
#"responsive"：使表格在不同屏幕尺寸下都能自适应显示。
...
`AFINN` 词典为单词赋予从 -5 到 +5 的分数，其中负分数越多表示情绪越消极，正分数越多表示情绪越积极。
`nrc` 词典选择二元分类：积极、消极、愤怒、期待、厌恶、恐惧、喜悦、悲伤、惊讶和信任，每个单词针对每种情绪赋予 1/0 的分数。换句话说，对于 `nrc` 词典，如果单词包含不止一种这样的情绪（例如，参见上面的“放弃”），则单词会出现多次。
`bing` 词典最简单，将单词简单地分为二元“积极”或“消极”类别。
...
totals <- tidy_tweets %>%
  mutate(obs=1) %>% #在每一行后面都加了一个1，方便计算
  group_by(date) %>%
...
tidy_tweets %>%
  ...
  #将 mordict 中的所有词汇用竖线 | 连接成一个字符串，形成一个正则表达式模式，例如 "death|illness|hospital|..."
  #ignore.case = T 忽略大小写
  group_by(date) %>%
  summarise(sum_mwords = sum(obs)) %>%
  full_join(totals, word, by="date") %>% #full_join 函数将前面统计的死亡相关词汇数据与 totals 数据框按照 date 列进行合并（就算filter掉了不包含死亡词语的日期，这一步也会把它们加回来
  mutate(sum_mwords= ifelse(is.na(sum_mwords), 0, sum_mwords),#使用 ifelse 函数将 sum_mwords 列中的 NA 值替换为 0
         pctmwords = sum_mwords/sum_words) %>% #计算每个日期下死亡相关词汇占总词汇的百分比，存储在新列 pctmwords 中
